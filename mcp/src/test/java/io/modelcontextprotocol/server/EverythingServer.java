...
public class EverythingServer {
	...
	public EverythingServer(McpServerTransportProvider transport) {
		...
		for (int i = 0; i < 10; i++) {
			...
			specs[i] = new McpServerFeatures.SyncResourceSpecification(
					Resource.builder().uri(uri).name("Test Resource").mimeType("text/plain")
		...
		this.server = McpServer.sync(transport).serverInfo(getClass().getName() + "-server", "1.0.0")
				.capabilities(
						ServerCapabilities.builder().logging().tools(true).prompts(true).resources(true, true).build())
				.toolCall(Tool.builder().name("echo").description("echo tool description").inputSchema(emptyJsonSchema)
						.build(), (exchange, request) -> {
							return CallToolResult.builder().addTextContent((String) request.arguments().get("message"))
									.build();
						})
				.toolCall(
						Tool.builder().name("add").description("add two integers").inputSchema(emptyJsonSchema).build(),
						(exchange, request) -> {
				...
				.toolCall(Tool.builder().name("sampleLLM").description("sampleLLM tool").inputSchema(emptyJsonSchema)
						.build(), (exchange, request) -> {
							...
							CreateMessageRequest cmRequest = CreateMessageRequest.builder().messages(List.of(sm))
									.systemPrompt("You are a helpful test server.").maxTokens(maxTokens)
									.temperature(0.7).includeContext(ContextInclusionStrategy.THIS_SERVER).build();
				...
				.toolCall(Tool.builder().name("longRunningOperation")
						.description("Demonstrates a long running operation with progress updates")
						.inputSchema(emptyJsonSchema).build(), (exchange, request) -> {
							...
							for (int i = 0; i < steps; i++) {
								...
								if (progressToken != null) {
									exchange.progressNotification(
											new ProgressNotification(progressToken, (double) i + 1, (double) steps,
													"progress message " + String.valueOf(i + 1)));
								}
							}
				...
				.toolCall(Tool.builder().name("annotatedMessage").description("annotated message").build(),
						(exchange, request) -> {
							...
							if (messageType.equals("success")) {
								...
								annotations = new Annotations(List.of(McpSchema.Role.USER, McpSchema.Role.ASSISTANT),
										1.0);
							} else if (messageType.equals("debug")) {
								annotations = new Annotations(List.of(McpSchema.Role.ASSISTANT), 0.3);
							}
							return CallToolResult.builder().addContent(new TextContent(annotations, "some response"))
									.build();
						})
				.prompts(List.of(new SyncPromptSpecification(
						new Prompt("simple_prompt", "Simple prompt description", null), (exchange, request) -> {
							return new GetPromptResult("description",
									List.of(new PromptMessage(Role.USER, new TextContent("hello"))));
						})))
				.resources(specs).build();
	}
